---
title: "WebAnalytics All Large State Uploading"
author: "TF"
date: "Feb 15, 2019"
output:
...
# Web Analytics Monthly Uploading Report -- for Large claim amount States
...
#Provider list updating is in all_sample1.rmd
all_provider = sqlQuery(webanalyticsAAGchannel, "SELECT * FROM Active_ClientProvider",stringsAsFactors = FALSE)
```

1.Assign state and Client group
```{r echo = FALSE, include = TRUE}
# round 11-------- 25 IA, KS, NE, OH, OK
SS= Sys.time()
state_y = c(m)
client_y = c(n) #A-H I-Q   R-Z/// I-L, M-Q
print(state_y)
print(client_y)
...
pre3startdate = paste(paste0("'", format(Sys.Date()-120,"%Y-%m-01" ), "'"), collapse = ',')
aagcon = set_connString('SQLAAG-L','InfinediAnalytics')
...
AND  acp.PracticeState = %s AND left(acp.clientcode,1) like %s
...
SELECT * FROM Jclm1", prestartdate, enddate, state_y,client_y, prestartdate, enddate, state_y,client_y), stringsAsFactors = FALSE)
e = Sys.time()
print(paste('Time for get all claim_table1 is:',e-s)) 
...
and cpc.receiveddate >= (%s) and cpc.receiveddate < (%s)
AND acp.PracticeState = %s AND left(acp.clientcode,1) like %s
AND cpcm1.AmountTypeId = 1 AND  cpcdd2.Datetypeid = 39 AND cpcm2.AmountTypeId = 5 ",prestartdate, enddate, state_y,client_y), stringsAsFactors = FALSE)
e = Sys.time()
...
and cpc.receiveddate >= (%s) and cpc.receiveddate < (%s) 
AND acp.PracticeState = %s AND left(acp.clientcode,1) like %s
and cpcd.ProviderTypeId = 1 and cpcp.PersonTypeId = 2 and cpci.IdentifierTypeId = 41", prestartdate, enddate, state_y,client_y), stringsAsFactors = FALSE)
e = Sys.time()
...
FROM (SELECT cpc.ClaimId,cpcdd.DiagnosisCode,cpcdd.DiagnosisTypeId AS DiagTypeId, ROW_NUMBER() OVER (PARTITION BY cpc.ClaimId ORDER BY cpcdd.DiagnosisTypeId, cpcdd.ClaimDiagnosisId) rnk
    FROM [ClaimProcessing].[dbo].[Claim] AS cpc
    INNER JOIN claimprocessing.dbo.ClaimDoctor as cpcd WITH(INDEX(IX_ClaimDoctor_ClaimId)) ON cpc.claimid = cpcd.claimid 
    INNER JOIN InfinediAnalytics.dbo.Active_ClientProvider AS acp with(index(IX_ActiveClientProvider_ProviderId_ClientCode)) ON cpcd.ProviderId = acp.ProviderId AND cpc.ClientCode = acp.ClientCode
...
WHERE rnk = 1",prestartdate, enddate, state_y,client_y), stringsAsFactors = FALSE)
e = Sys.time()
...
### Chunk 1.2 Combine table, format Cleaning
```{r echo=FALSE, include=TRUE}
...
```
### Chunk 1.3 Getting Existed Patient_tb(for the same client_y)
```{r echo=FALSE, include=TRUE}
...
where acp.PracticeState = %s AND left(acp.clientcode,1) like %s", state_y, client_y), stringsAsFactors = FALSE)
#
#
...
### Chunk 1.4 Find the New patient
```{r echo=FALSE, include=TRUE}
s = Sys.time()
...
# first find the max id of patientid for each clientCode, since each patientid is arranged to each clientcode
      ...
      max_patid = pat_tb %>%
         ...
         group_by(ClientCode) %>%
         summarise(max_patid = max(old_id)) 
...
new_pat %>%
  ...
  summarise(cnt = n_distinct(PatientId)) %>%
  filter(cnt>1)
print('Check Criteria2: Whehter PatientId is assigned to multiple people: Expect result: 0 X 8')
new_pat %>%
 group_by(PatientId) %>%
   summarise(cnnt = n_distinct(PatFirstName)) %>%
   filter(cnnt>1)
e= Sys.time()
e-s
 # new_clm[is.na(new_clm$PatGender),]
```
### chunk 1.5 Clean and Formatting and then Push new patient_tb to SQL(R ->SQL)
```{r echo=FALSE, include=TRUE}
s= Sys.time()
...
#push_data(aagcon, new_push_patient, "dbo.V1Patient_tb", append = TRUE, showprogress = TRUE)
```

### chunk 1.6 Assign other ids and clm_length for clm_LOAD
```{r echo=FALSE, include=TRUE}
s=Sys.time()
...
```
### chunk 1.7 Deal with Parent Claims and release R memory
```{r echo=FALSE, include=TRUE}
s= Sys.time()
drop_parent_func <- function(tb){
   t2 = tb %>%
      group_by(TraceNumber) %>%
      summarise(latest_clm = max(ClaimId)) %>%
...
```{r echo = FALSE, include = TRUE}
Episode_tb = sqlQuery(webanalyticsAAGchannel, sprintf("WITH tt as (
SELECT DiagId,EpisodeId, SrvEndDATE AS LastSrvDate, row_number() over(partition by DiagId order by SrvEndDate desc) as n 
FROM [InfinediAnalytics].[dbo].[V1ClaimProcessing] as cp 
inner join [InfinediAnalytics].[dbo].Active_ClientProvider as acp on cp.ClientCode = acp.ClientCode and cp.ProviderId = acp.ProviderId WHERE acp.PracticeState in (%s)
)
 select * from tt where N = 1", state_y), stringsAsFactors = FALSE)
...
library(tidyr)
 Episode_tb = Episode_tb %>% 
   ...
   mutate(LastSrvDate = as.Date(as.character(LastSrvDate), format = "%Y-%m-%d"), 
          ...
          old_id  = as.numeric(id)) %>%
   select(DiagId, LastSrvDate, old_id) %>%
   distinct() %>%
   as.data.frame()
```
...
new_epiid = new_epi %>%
   left_join(ep_tb, by = c('DiagId')) %>%
   filter(is.na(old_id)) %>%
   mutate(id = ifelse(is.na(old_id), 1, old_id)) %>%
   mutate(EpisodeId = gsub('\\s+', '', paste(id, DiagId, sep = "_"))) %>%
   select(-c(LastSrvDate, old_id))  

  if(nrow(ep_tb) != 0){ #exist clients have updated_epiid
    update_epiid = new_epi %>%
      left_join(ep_tb, by = 'DiagId') %>%
      filter(!is.na(old_id)) %>%
      group_by(DiagId) %>%
      mutate(row_id = row_number(),
             epi_int = ifelse(row_id == 1, as.numeric(SrvStartDate-LastSrvDate, units = 'days'),
                                           as.numeric(SrvStartDate-lag(SrvEndDate, units = 'days')))) %>%
      mutate(id = ifelse(row_id == 1 & epi_int >=60, as.numeric(old_id) + 1, old_id))
    #update
    for(i in (1: nrow(update_epiid))){
     ...
     update_epiid = update_epiid %>%
      distinct()%>%
      mutate(EpisodeId = gsub('\\s+', '', paste(id, DiagId, sep ="_"))) %>%
      select(c(DiagId, SrvStartDate, SrvEndDate, EpisodeId))%>%
      as.data.frame()
    new_clm3 = bind_rows(new_epiid, update_epiid) %>%
...
new_clm3 = episode_func(Episode_tb, new_clm2)
e = Sys.time()
print(paste("Time for assign and update episodeid take: ", e-s))
```
###1.9 Formatting then Push to SQL ClaimProcessing_LOAD & ClaimProcessing(R ->SQL)
```{r echo=FALSE,include=TRUE}
s = Sys.time()
colnames(new_clm3)[colnames(new_clm3) =='PracticeState'] = 'State'
...
print("V1ClaimProcessing_LOAD")
push_data(devcon, new_clm1_LOAD, "dbo.V1ClaimProcessing_LOAD", append = TRUE, showprogress = TRUE)
print("V1ClaimProcessing")
push_data(devcon, clm, "dbo.V1ClaimProcessing", append = TRUE, showprogress = TRUE)

#push_data(aagcon, new_clm1_LOAD, "dbo.V1ClaimProcessing_LOAD", append = TRUE, showprogress = TRUE)
#push_data(aagcon, clm, "dbo.V1ClaimProcessing", append = TRUE, showprogress = TRUE)
e = Sys.time()
print(paste('Push to V1ClaimProcessing and V1ClaimProcessing_LOAD:',e-s))
```
...
memory.size()
rm(all_clmJ1, all_clmJ2, all_clmJ3, all_clmJ4, new_clm,clm, Episode_tb,  new_clm1, new_clm1_LOAD, new_clm2, new_clm3, new_epi, new_epiid, new_pat, new_push_patient, new_result1, patient_tb, update_epiid, update_epiid1)
gc()
memory.size()
```
...
cm.CoreModuleName, cet.ErrorTag, cem.ErrorModeName,cet.ErrorDesc, AmountValue as ErrorClaimAmount
  FROM ClaimProcessing.dbo.claim as cpc
   INNER JOIN ClaimProcessing.dbo.ClaimAmount as ca on cpc.Claimid = ca.claimid and AmountTypeId = 1
  INNER JOIN ClaimProcessing.dbo.ClaimDoctor as cpcd WITH(INDEX([IX_ClaimDoctor_ClaimId])) ON cpc.Claimid = cpcd.Claimid
...
```
# Clean up Error Message and generate Error Reason
```{r echo=FALSE,include=TRUE}
err_mess_func <- function(error_tb){
  #put blank before upper case letter
     error_tb$ErrorMessage1 = ifelse(error_tb$ErrorTag !='Legacy Error', error_tb$ErrorDesc, error_tb$ErrorMessage)
     #1.delete all inside ><
...
#push_data(aagcon, clm_error1, "dbo.V2ClaimError", append = TRUE, showprogress = TRUE)
```
```{r}
rm(clm_error, clm_error1)
gc()
```
### chunk 1.12 Get Match Claim for ERA
```{r echo=FALSE,include=TRUE}
...
where acp.PracticeState = %s AND left(acp.clientcode,1) like %s and cpc.receiveddate >= (%s) and cpc.receiveddate < (%s) ", state_y, client_y, pre3startdate, enddate), stringsAsFactors = FALSE)
clm_match = change_clmtype_func1(all_provider, clm_match)
print("How many claims for each clientcode will be used to match ERA: ")
clm_match %>%
  group_by(ClientCode) %>%
  summarise(cnt = n_distinct(TraceNumber)) %>%
  arrange(desc(cnt))
```


## ERA
### 2.1.1 Getting ERA (SQL -> R)
```{r echo=FALSE,include=TRUE}
...
new_era = sqlQuery(erachannel, sprintf("with result as (
   SELECT DISTINCT acp.PracticeState, ee.CliCode AS CliCode, CONCAT(CliCode,'', PatAcctNum) as Combined,CAST(ee.MessageTime AS DATE) AS EraMessageDate, CAST(et.CheckEFTDate AS DATE) AS PaidDate, ec.Claimid AS EraClaimId, ec.TraceNumber, ec.ClaimStatus AS EraClaimStatus,pr.PayerID, CONCAT(pr.PayerSt,' ', pr.PayerName) AS StPayerName,  upper(ec.PatLastName) as PatLastName, upper(ec.PatFirstName) as PatFirstName, ec.PatAcctNum, ec.PatInsID,ec.ICN, es.SrvLineId AS EraSrvLineId, es.RenProvId, es.SrvStartDate, es.SrvEndDate, es.POS, es.ProcCode, es.Mod1, es.Mod2,es.Mod3, es.Billed AS srv_Billed, es.Allowed AS srv_Allowed, es.Deduct AS srv_Deduct, es.COINS AS srv_COINS, es.ProvPaid AS srv_ProvPaid, es.Reduced AS srv_Reduced, case when es.QtyPaid =0.0 then es.qtybilled else es.qtypaid end AS srv_Qty, es.QtyPaid as srv_QtyPaid, es.QtyBilled as srv_QtyBilled
...
AND acp.PracticeState = %s AND left(acp.clientcode,1) like %s
 ...
 select distinct * from endResult", prestartdate, enddate, state_y, client_y, pre3startdate, enddate, state_y,client_y, pre3startdate, enddate, state_y,client_y), stringsAsFactors = FALSE)
e= Sys.time()
new_era = change_eratype_func(new_era)
print(paste("Time for get ERA data take: ",e-s))
```
#2.1.2 get claim adj and srv adj
```{r echo=FALSE,include=TRUE}
...
eraadj = sqlQuery(erachannel, sprintf("SELECT DISTINCT ec.ClaimId as EraClaimId,
   ...
   WHERE ee.Status !=4 --AND ec.ClaimStatus !=4  --not Rejected nor Duplicate
   AND ee.MessageTime >= (%s) and ee.MessageTime < (%s)AND acp.PracticeState = %s AND left(acp.clientcode,1) like %s
 ", prestartdate, enddate, state_y, client_y), stringsAsFactors = FALSE)
eraadj = change_eraadj_type(eraadj)
print(paste("Time for getting ERA Adj takes:", e-s))
```
...
```{r echo=FALSE,include=TRUE}
 era_step1_func <- function(era, clm_final){
   ...
   print(paste('original EraClaimId rows:',length(unique(era$EraClaimId))))
 ...
 #DROP the rest LegacyProviderId
   ...
   print(paste('After filling ProviderId, there are :',length(unique(era1$EraClaimId)), 'ERA rows left.'))
   return(era1)
 }
...
era_step12_func <- function(tb1, clm_tb){
  #4.match with clmkey
   ...
   print(paste('After Join with claim, there are:',length(unique(era_load$EraClaimId)), 'ERA rows left.'))
   return(era_load)
...
era_step2_func <- function(era_step1_result){
  ...
  #aagcon = set_connString('SQLAAG-L','InfinediAnalytics')
  #push_data(aagcon, v2era_load, "dbo.V2Era_LOAD", append = TRUE, showprogress = TRUE)
  
  # #phase 3, need the claimstatus ==22
  # devcon = set_connString('SQLDEV2014\\MSSQLDEV2014','InfinediAnalytics')
  # push_data(devcon, v2era_load, "dbo.V3era1", append = TRUE, showprogress = TRUE)
...
era_step3_func<- function(era_2, eraadj){
  ...
  V1erabase = era_3 %>%
    select( c("CliCode","EraMessageDate","PaidDate" , "EraClaimId","TraceNumber","EraClaimStatus", "PayerID","StPayerName", "PatAcctNum","PatInsID","ICN","EraSrvLineId","RenProvId","SrvStartDate","SrvEndDate", "POS","ProcCode","Mod1", "Mod2","srv_Billed","srv_Allowed","srv_Deduct","srv_COINS","srv_ProvPaid","srv_Reduced", "srv_Qty","EraMonth", "ProcedureId","ProviderId", "PatientId")) %>%
    distinct()
  ...
  # aagcon = set_connString('SQLAAG-L','InfinediAnalytics')
  # push_data(aagcon, V1erabase, "dbo.V1EraBase", append = TRUE, showprogress = TRUE)
  ...
  # aagcon = set_connString('SQLAAG-L','InfinediAnalytics')
  # push_data(aagcon, V2erabase, "dbo.V2EraBase", append = TRUE, showprogress = TRUE)
  #print(V2erabase)
  return(era_3)
}
